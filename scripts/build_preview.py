...
.toplinks { display: grid; gap: .5rem; margin: 0 0 1.2rem; }
.rule { height: 1px; margin: 0 0 1.2rem; border: 0; background: var(--line); }
.tl {
...
.tl b { font-size: 1rem; font-weight: 750; letter-spacing: -.01em; }
.tl span {
...
/* Felix puts the week's links at the bottom of the post, under the blurb.
   Readers want them first, so hoist any line carrying a URL to the top and
   keep the prose underneath. */
...
function openNote(id) {
  ...
  document.getElementById("s-text").innerHTML =
    ...
    + (prose ? `<div class="prose">${linkify(prose)}</div>` : "");

...
:root {
  ...
  --brandgrad: linear-gradient(135deg,#0b5f5c 0%,#0f6f92 52%,#1b93cf 100%);
...
@media (prefers-color-scheme: dark) {
  :root:not([data-theme="light"]) {
    ...
    --brandgrad: linear-gradient(135deg,#0a4f4d 0%,#0d5f7d 52%,#1780b4 100%);
  }
}
...
.lead {
  ...
  border: 0; box-shadow: var(--shadow-lift);
  transition: box-shadow .18s ease, transform .18s ease;
}
/* A soft highlight keeps the flat gradient from reading as a solid block. */
.lead::after {
  content: ""; position: absolute; inset: -40% -30% auto auto; width: 22rem; height: 22rem;
  background: radial-gradient(circle, rgba(255,255,255,.16) 0%, transparent 62%);
  pointer-events: none;
}
.lead:hover { transform: translateY(-3px); box-shadow: 0 20px 44px -16px rgba(4,19,26,.5); }
...
.ygroup {
  display: flex; align-items: center; gap: .75rem; padding: .8rem 1.3rem;
  background: var(--panel-2); border-bottom: 1px solid var(--line);
  position: sticky; top: 0; z-index: 2;
}
.ygroup::before { content: ""; width: 3px; height: 1.05rem; border-radius: 2px; background: var(--brand-2); }
.ygroup b { font-size: 1.05rem; font-weight: 800; letter-spacing: -.01em; color: var(--brand); }
...
.qbar { flex: none; width: 3px; align-self: stretch; margin-right: .15rem; opacity: .85; }
.q1 { background: #1b93cf; } .q2 { background: #0f8f8a; }
.q3 { background: #4f8fd0; } .q4 { background: #12708f; }
